...
export class WorkspaceService {
  ...
  static async listDriveFiles(accessToken: string, queryFilter: string = ''): Promise<DriveFile[]> {
    ...
    const res = await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name,mimeType,webViewLink,iconLink,thumbnailLink,modifiedTime)&pageSize=20`, {
  ...
  static async fetchGmailMessages(accessToken: string, queryStr: string = 'subject:Classroom OR subject:Course'): Promise<any[]> {
    const res = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages?q=${encodeURIComponent(queryStr)}&maxResults=10`, {
      headers: { Authorization: `Bearer ${accessToken}` }
    });
    if (!res.ok) return [];
    const data = await res.json();
  ...
  static async fetchTasks(accessToken: string): Promise<GoogleTask[]> {
    try {
      const res = await fetch('https://tasks.googleapis.com/tasks/v1/users/@me/lists/@default/tasks?maxResults=20', {
        headers: { Authorization: `Bearer ${accessToken}` }
      });
      if (!res.ok) return [];
      const data = await res.json();
